...
class FluidProperty {
  ...
  constructor(el, name, valuesByBreakpoint, breakpoints, computedStyleCache, boundClientRectCache, fs) {
    ...
    this.isBgPos =  propertyName.includes ('background-position');
...
/*
  ...
  processBgPos (values)
  {
    if(!values.includes ('top') && !values.includes ('left') && !values.includes ('right') && !values.includes ('bottom'))
      return values;

    const rect = getCachedBoundingClientRect (this.el, this.boundClientRectCache);
    const arr = [];

    for(const [index, v] of values.entries())
    {
      const nextV = values[index + 1];
      ...
      switch(v)
      {
        ...
        case "left":
          ...
          break;
        case "bottom":
          arr.push (rect.height + nextV);
        break;
        case "right":
          arr.push (rect.width + nextV);
        break;
      }
    }
  ...
  getValues(breakpointIndex, currentWidth) {
    ...
    let maxValues = breakpointValues.maxValues.map((maxVal, index) => computeVal(maxVal, breakpointValues.maxUnits[index], this.name, this.el, this.computedStyleCache, this.boundClientRectCache));
    
    if (this.isBgPos)
      maxValues = this.processBgPos (maxValues)
    
    if(progress >= 1)
      values = maxValues;
    else 
    {
      let minValues = breakpointValues.minValues.map((maxVal, index) => computeVal(maxVal, breakpointValues.maxUnits[index], this.name, this.el, this.computedStyleCache, this.boundClientRectCache));
      
      if (this.isBgPos)
        minValues = this.processBgPos (minValues);

      values = minValues.map((val, index) => {
      ...
      if(Array.isArray (val) && val[0] === 'break')
      {
          return val[1];
      }
