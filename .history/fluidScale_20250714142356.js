...
/*
  ...
  getValues(breakpointIndex, currentWidth) {
    ...
    if (this.propertyName.includes ('background-position'))
      console.log (breakpointValues.minValues);
    if(progress >= 1)
      ...
      for (const selector of selectors)
      {
        ...
        for (const fluidPropertyName of fluidPropertyNames) {
         ...
         spanEnd = spanEnd || rule.style.getPropertyValue ('--span-end')?.split (',').map (s => s.trim()) || [];
          ...
          if (variableName.startsWith ('background'))
            console.log (variableName, allCalcsParsed);
          let minValues
          let maxValues;
          let isCombo = false;
